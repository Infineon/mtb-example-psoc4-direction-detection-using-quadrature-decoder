# PSOC&trade; 4 : Direction detection using quadrature decoder
...
[Provide feedback on this code example.](https://yourvoice.infineon.com/jfe/form/SV_1NTns53sK2yiljn?Q_EED=)
...
8. For [KIT_PSOC-HVPA-SPM1_LITE](https://www.infineon.com/design-resources/finder-selection-tools/evaluation-board), connect a jumper wire from PWM_phiA (P3.2) output to phiA (P0.4) QuadDec input, and connect another jumper wire from PWM_phiB (P0.3) output to phiB (P3.0) QuadDec input.
...
The following example clones the "[mtb-example-psoc4-direction-detection-using-quadrature-decoder](https://github.com/Infineon/mtb-example-psoc4-direction-detection-using-quadrature-decoder)" application with the desired name "QuadDec" configured for the [*CY8CKIT-149*](https://www.infineon.com/CY8CKIT-149) BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id CY8CKIT-149 --app-id mtb-example-psoc4-direction-detection-using-quadrature-decoder --user-app-name QuadDec --target-dir "C:/mtb_projects"
   ```
...
2. In the *main.c* file, find the line “#define ROTATION” and set the value for desired rotation as CLOCKWISE, COUNTER_CLOCKWISE, NO_ROTATION. For example, to select clockwise rotation set #define ROTATION to CLOCKWISE.
...
4. On successful programming,

   - For [CY8CKIT-041S-MAX](https://www.infineon.com/CY8CKIT-041S-MAX) and [CY8CKIT-045S](https://www.infineon.com/CY8CKIT-045S), LED1 turns on when the direction is clockwise; LED2 turns on when the direction is counter-clockwise.

   - For [CY8CKIT-145-40XX](https://www.infineon.com/CY8CKIT-145-40XX) and [CY8CKIT-149](https://www.infineon.com/CY8CKIT-149), LED5 turns on when the direction is clockwise; LED6 turns on when the direction is counter-clockwise.

   - For [KIT_PSOC4-HVMS-64K_LITE](https://www.infineon.com/cms/en/product/evaluation-boards/kit_psoc4-hvms-64k_lite/),
   [KIT_PSOC4-HVMS-128K_LITE](https://www.infineon.com/cms/en/product/evaluation-boards/kit_psoc4-hvms-128k_lite/),[KIT_PSOC4-HVPA-144K_LITE](https://www.infineon.com/cms/en/product/evaluation-boards/kit_psoc4-hvpa-144k_lite/)and [KIT_PSOC-HVPA-SPM1_LITE](https://www.infineon.com/design-resources/finder-selection-tools/evaluation-board), LED7 turns on when the direction is clockwise; LED6 turns on when the direction is counter-clockwise. Both LEDs turn on when no rotation is detected.
...
For [KIT_PSOC4-HVMS-64K_LITE](https://www.infineon.com/cms/en/product/evaluation-boards/kit_psoc4-hvms-64k_lite/), if the rotation is clockwise, LED7 is ON; if it is counter-clockwise, LED6 is ON.

For [KIT_PSOC4-HVMS-128K_LITE](https://www.infineon.com/cms/en/product/evaluation-boards/kit_psoc4-hvms-128k_lite/), if the rotation is clockwise, LED7 is ON; if it is counter-clockwise, LED6 is ON.

For [KIT_PSOC4-HVPA-144K_LITE](https://www.infineon.com/cms/en/product/evaluation-boards/kit_psoc4-hvpa-144k_lite/), if the rotation is clockwise, LED7 is ON; if it is counter-clockwise, LED6 is ON.

For [KIT_PSOC-HVPA-SPM1_LITE](https://www.infineon.com/design-resources/finder-selection-tools/evaluation-board), if the rotation is clockwise, LED7 is ON; if it is counter-clockwise, LED6 is ON.
...
**Table 1. Application resources**
 ...
 Led 1 (BSP) | LED1 | User LED to show output
 Led 2 (BSP) | LED2 | User LED to show output
...
Document title: *CE236460* - *PSOC&trade; 4: Direction detection using quadrature decoder*
 ...
 1.6.0   | Added support for KIT_PSOC-HVPA-SPM1_LITE and updated to support ModusToolbox&trade; v3.7 and operation changes for KIT_PSOC4-HVPA-144K_LITE
<br>
